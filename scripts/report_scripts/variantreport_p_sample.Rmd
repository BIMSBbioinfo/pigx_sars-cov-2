...
csv_output_dir <- "path/to/csv_dir"
``` 
...
# check if there is already a file for the output
# if yes: load that file
    # TODO
    # output_variant_plot <- read.table("output/file", sep = "\t", header = TRUE)
# if not:
output_variant_plot <- data.frame(samplename = samplename, # brauche ich als input von sample sheet 
                                  dates = date,
                                  time = time,
...
# get all possible variants from sig_mutations.df
all_variants <- colnames(msig_all)

# add columns for all possible variants to the dataframe
for (variant in all_variants) {
  output_variant_plot[,variant] <- NA
}
# adding new row
#new_row <- c(samplename = "sample2", dates = "2021/04/23", location_name = "Blob", WT = as.character(0.23), b117 = as.character(0.67))
meta_data <- c(
...
for (i in all_variants){
  if (i %in% df$name){
      variant_values <- c(variant_values, i = )
        # append a row for that variable
        # output_variant_plot[nrow(output_variant_plot)+1,i] <- df$value[df$name == i]

}
  }

output_variant_plot <- bind_rows(output_variant_plot, meta_data)
# get rownumber for sample
sample_row <- which(grepl(samplename, output_variant_plot$samplename))

...
# deduplicate dataframe
# fixme: This can be simplified and it also does reduntant repetition of loops

msig_stable_transposed <- t (msig_stable)
msig_stable_transposed <- as.data.frame(cbind(variants = colnames(msig_stable), msig_stable_transposed))

dupes_variants <- duplicated (msig_stable_transposed[,-1], fromLast=TRUE)
allDupes_variants <- dupes_variants | duplicated (msig_stable_transposed[,-1], fromLast=FALSE)

# fixme isn't that a case for "distinct" ?
if (any (allDupes_variants)) {
   var_deduped_transposed <- msig_stable_transposed[!dupes_variants,]
}
var_dedup <- as.data.frame(t(var_deduped_transposed))
...
# fixme: I think this should be done with some apply() function? 
for (variant in rownames(msig_stable_transposed)[-1]) {
  
  var_deduped_transposed <- dedupeVariants(variant, msig_stable_transposed, var_deduped_transposed)
}

msig_stable_unique <- as.data.frame(t(var_deduped_transposed[,-1])) # fixme: proper variable handling
```
```{r deconvolution, message=F, warning=F, include = FALSE, eval = executeDeconvolution}
sig <- apply(msig_stable_unique[,-1], 2 , function(x) {
    as.numeric(as.character(x))
})

#sig <- as.numeric(msig_stable_unique[,-1])

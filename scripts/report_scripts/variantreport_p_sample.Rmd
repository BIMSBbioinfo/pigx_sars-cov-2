...
time <- as.character(sample_sheet[name==sample_name]$time)
...
signature mutations characterising variants of concern (VOC) of SARS-CoV-19 provided by [outbreak.info](https://outbreak.info/situation-reports) 
...
######
b16172 <- read.table(file.path(location_sigmuts,"B16172_outbreakinfo_mutation_report_data_2021-05-26.tsv"),
                sep="\t",header=T)
b16172$alt_aa <- na_if(b16172$alt_aa, '')
b16172$alt_aa[is.na(b16172$alt_aa)] <- "-"
b16172$AA_mut <- paste0(b16172$ref_aa, b16172$codon_num, b16172$alt_aa)
######
...
The following tables list the called SNVs. Table 1 \@ref(fig:sig_table) shows the mutations matching signature mutations for given SARS-CoV-2 
variants. Table 2 \@ref(fig:nonsig_table) shows all other mutations without any match. The columns are described as follows:    
...
```
# Deconvolution 
Deconvolution is done by using robust regression. It relies on signature mutations of each virus strain/variants. It aims 
...
```
The following bar plot shows the percentage of each VOCs in the sample as predicted by the deconvolution model. If it is
not possible to identify a unique VOC from the reported mutation, the column description shows all VOCs which are realted
to the signature mutation. 
...
```{r, include = F}
snvs <- readLines(snvfile) %>% paste0(collapse="\n") %>% base64_urlencode()
```
Called SNVs:  
...
vep <- readLines(vepfile) %>% paste0(collapse="\n") %>% base64_urlencode()
```

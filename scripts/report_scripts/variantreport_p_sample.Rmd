...
vepfile <- file.path(variants_dir,"/",sample_name,"_vep_sarscov2_parsed.txt")
snvfile <- file.path(variants_dir,"/",sample_name,"_snv.csv")
...
muts <- c(match_unique.df$AA_mut)
# get bulk frequency values
bulk <- as.numeric(match_unique.df$freq)
...
# 1. collapsing identical columns

# transposing in order to be able to use the distinct functions
msig_transposed <- as.data.frame(t(msig_stable[,-1]))
# collaps identical columns and transpose back
msig_stable_unique <- data.frame(muts = msig_stable[,1],t(msig_transposed %>% distinct(.keep_all = TRUE)))

# 2. collaps the column names and rename the columns
colnames_collapsed <- getIdentCols(msig_stable_complete)
colnames(msig_stable_unique) <- colnames_collapsed
```
...
# output_variant_plot <- data.frame(date = "21/03/2", # brauche ich als input von sample sheet 
#                                   location = character())
# 
# 
# # 1. get all possible variants from sig_mutations.df
# # 2. check if variant has a unique row in df (from plotting)
# #   - if yes: write value in "values"
# #   - if not: write NA
# write.csv2(vepfile.df, "./file.csv")
```
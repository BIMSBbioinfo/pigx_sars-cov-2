...
if ( !(length(coverages.df) == 0)){
    ...
    good_samples.df <- coverages.df %>% filter( as.numeric(proport_muts_covered) >= 1) # TODO stringecy should be given by user, maybe even through visualization
...
if (nrow(approved_var_plot) > 0) { # check if filtered dataframe has actual values
  ...
  # TODO change order or "variable" so that "Others" is always displayed on top

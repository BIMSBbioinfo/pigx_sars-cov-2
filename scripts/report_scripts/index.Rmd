...
COVERAGES_FOUND <- length(coverages_df) > 0
...
```{r sample_quality_score_warning, results = "asis", warning=TRUE, eval=!COVERAGES_FOUND}
...
```{r title_lm_table, results = 'asis', eval=MUTATIONS_FOUND}
cat(
...
```{r lm_table_warning, results = 'asis', warning=TRUE, eval=MUTATIONS_FOUND && !RUN_MUTATION_REGRESSION}
warning(paste(
...
```{r compute_linear_regression_tables, eval=RUN_MUTATION_REGRESSION}
...
```{r mut_reg_tables_caption, results="asis", eval=SIGN_MUTS_FOUND}
cat(
...
```{r mut_reg_tables_warning, results="asis", warning=TRUE, eval=!SIGN_MUTS_FOUND}
warning(
...
```
```{r table_sample_quality_score, eval=COVERAGES_FOUND}
# TODO refine column names --> e.g "coverage in percent",
bad_samples_df <- bad_samples_df %>% slice_head(n = 20)
create_dt(bad_samples_df)
```

```{r table_sample_quality_score_warning, warning=TRUE, eval=!COVERAGES_FOUND}
warning("\n No coverage values found.")
```

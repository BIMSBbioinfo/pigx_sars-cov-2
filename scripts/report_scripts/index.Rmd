...
df_var <- read.table(params$variants_csv, sep = "\t", header = TRUE,
  check.names = FALSE)
df_mut <- read.table(params$mutations_csv, sep = "\t", header = TRUE,
  check.names = FALSE)

# NOTE: These separators need to be changed before file io can be done via csv
mut_str_sep <- ":"
```
...
sigmuts_df <- read.csv(mutation_sheet, header = TRUE) %>%
  ...
  mutate(across(everything(), ~str_replace(.x, str_glue('^[^{mut_str_sep}]*{mut_str_sep}'), '')))
...
if (nrow(mutations_sig) > 0) {
  ...
  mutations_sig <- mutations_sig %>%
    ...
    mutate(sigflag = ifelse(any(
      sigmuts_df == str_split(mutation, str_glue("{mut_str_sep}{mut_str_sep}"))[[1]][2]
    ), "SigMut", ""))
...
melted <- melted %>%
  ...
  rowwise() %>%
  mutate(sigflag = ifelse(any(
    sigmuts_df == str_split(variable, str_glue(
      "{mut_str_sep}{mut_str_sep}"
    ))[[1]][2]
  ), "SigMut", "")) %>%
  ungroup()
  ...
  mutate(
    sigflag = ifelse(
      any(sigmuts_df == str_split(
        variable, str_glue("{mut_str_sep}{mut_str_sep}")
      )[[1]][2]),
      "SigMut", NULL
    )
  )
...
```{r table_ample_quality_score, warning=TRUE}
if (!(length(coverages.df) == 0)) {

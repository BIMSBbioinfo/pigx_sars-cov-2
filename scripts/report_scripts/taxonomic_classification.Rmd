...
```{r libs, echo=FALSE}
suppressMessages(suppressWarnings(library(DT))) # why is this here? Do we still need it?
```
Sample: `r params$sample_name`

# Classification of unaligned reads

In order to check the sample and alignment quality the unaligned reads are analysed with the
...
colnames(table) <- c("percent", "num_fragments", "num_fragments_taxon", "rank", "NCBI_ID", "taxon_class")
datatable(table)
```
